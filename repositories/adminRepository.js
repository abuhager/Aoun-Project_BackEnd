...
// مخصص لـ queries الأدمن — بعيداً عن adminService
...
exports.unbanUser = (userId) =>
  User.findByIdAndUpdate(
    userId,
    { $unset: { isBanned: '', banReason: '', bannedBy: '' } },
    { new: true }
  );
...
exports.resolveReport = (reportId, adminId, action) =>
  Report.findByIdAndUpdate(
    reportId,
    { $set: { status: 'resolved', resolvedBy: adminId, action } },
    { new: true }
  );

// ── السجلات ───────────────────────────────────────────────────
exports.logAdminAction = ({ adminId, action, targetId, targetModel, details }) =>
  AdminLog.create({ adminId, action, targetId, targetModel, details });
